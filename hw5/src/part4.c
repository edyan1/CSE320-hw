...
/*pseudocode: writer priority
begin read: 
reader lock
increment blocking reader counter b
if b=1 (went from 0 to 1) then lock global
reader unlock

end read:
reader lock
decrement b
if b=0 (last reader finished) then unlock global
reader unlock
...
pthread_mutex_t readLock; //mutex lock for read counter
